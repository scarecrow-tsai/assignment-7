...
def mnist_dataset(dataset_path, is_train, image_transforms):
    return datasets.MNIST(
        root=dataset_path, train=is_train, download=True, transform=lambda x: image_transforms(x)["image"],
    )


def cifar10_dataset(dataset_path, is_train, image_transforms):
    return datasets.CIFAR10(
        root=dataset_path, train=is_train, download=True, transform=image_transforms,
    )
